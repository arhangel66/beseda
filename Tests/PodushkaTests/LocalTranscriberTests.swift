...
@Test func transcribingWithoutTheModelFileFailsWithTheTypedError() async throws {
    let directory = FileManager.default.temporaryDirectory
        .appendingPathComponent("podushka-transcriber-\(UUID().uuidString)", isDirectory: true)
    let transcriber = LocalTranscriber(paths: AppPaths(dataDirectory: directory))

    await #expect(throws: PodushkaError.self) {
        try await transcriber.start()
    }
    do {
        _ = try await transcriber.start()
    } catch {
        #expect(error.localizedDescription == PodushkaError.runtimeMissingMessage)
    }
}
