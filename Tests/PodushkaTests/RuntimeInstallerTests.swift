...
private struct Harness {
    ...
    init() throws {
        directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("podushka-runtime-\(UUID().uuidString)", isDirectory: true)
...
@Test func aFailedDownloadStopsTheChainAndKeepsItsMessage() async throws {
    let harness = try Harness()
    defer { harness.tearDown() }
    let downloads = Downloads(payload: harness.payload, failure: PodushkaError.processFailed("Сервер ответил 404"))
